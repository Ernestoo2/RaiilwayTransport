...
const Tickets: React.FC<TicketsProps> = ({ tickets }) => {
  return (
    <div>
      ...
      <div className="mt-6">
        {tickets.map((ticket) => (
          <div key={ticket.id} className="flex items-center justify-between py-4 px-auto mb-4 rounded shadow">
            <div className="flex items-center pl-2 space-x-2">
              ...
              <div>
                <p className="font-bold text-base">{ticket.departure} → {ticket.arrival}</p>
            ...
            <button className="px-2 py-2 text-white bg-green-500 rounded">Download Ticket</button>
          </div>
        ))}
      </div>
    </div>
